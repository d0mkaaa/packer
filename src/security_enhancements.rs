...
/// Enhanced security scanner with real-time threat intelligence
...
impl AdvancedSecurityScanner {
    ...
    /// Initialize vulnerability feeds from multiple sources
    ...
    /// Update all vulnerability feeds with latest data
    ...
    /// Update a specific vulnerability feed
    pub async fn update_feed(&self, feed_name: &str) -> PackerResult<()> {
        ...
        if let Some(mut feed) = feed_info {
            ...
            return Err(PackerError::SecurityError(format!("Feed {} not found", feed_name)));
        }
    ...
    async fn fetch_nvd_vulnerabilities(&self) -> PackerResult<Vec<EnhancedVulnerability>> {
        ...
        let url = "https://services.nvd.nist.gov/rest/json/cves/2.0?resultsPerPage=2000&startIndex=0";
        
        let response = client.get(url).send().await?;
        if !response.status().is_success() {
            return Err(PackerError::NetworkError(format!("NVD API error: {}", response.status())));
        }
    ...
    async fn fetch_github_vulnerabilities(&self) -> PackerResult<Vec<EnhancedVulnerability>> {
        ...
        if !response.status().is_success() {
            return Err(PackerError::NetworkError(format!("GitHub API error: {}", response.status())));
        }
    ...
    async fn fetch_cisa_vulnerabilities(&self) -> PackerResult<Vec<EnhancedVulnerability>> {
        ...
        let response = client.get(url).send().await?;
        if !response.status().is_success() {
            return Err(PackerError::NetworkError(format!("CISA API error: {}", response.status())));
        }
        ...
        if let Some(vulns) = data["vulnerabilities"].as_array() {
            for vuln in vulns.iter().take(500) {
                if let Some(cve_id) = vuln["cveID"].as_str() {
                    ...
                    let due_date = vuln["dueDate"].as_str()
    ...
    async fn fetch_arch_vulnerabilities(&self) -> PackerResult<Vec<EnhancedVulnerability>> {
        ...
        let response = client.get(url).send().await?;
        if !response.status().is_success() {
            return Err(PackerError::NetworkError(format!("Arch Security API error: {}", response.status())));
        }
        ...
        if let Some(advisories) = data.as_array() {
            for advisory in advisories.iter().take(200) {
                if let Some(advisory_id) = advisory["id"].as_str() {
                    ...
                    let packages = advisory["packages"].as_array()
        ...
        info!("Fetched {} Arch Linux security advisories", vulnerabilities.len());
        Ok(vulnerabilities)
    }
    ...
    /// Enhanced package vulnerability scanning with real-time threat intelligence
    pub async fn enhanced_scan_package(&self, package: &Package) -> PackerResult<EnhancedScanResult> {
        info!("Performing enhanced security scan for package: {}", package.name);
        
        // Get vulnerabilities from all feeds
        let vulnerabilities = self.get_package_vulnerabilities(&package.name, &package.version).await?;
        
        // Calculate risk scores
        let risk_score = self.risk_calculator.calculate_comprehensive_risk(
            &vulnerabilities,
            package,
    ...
    /// Get vulnerabilities for a specific package from all feeds
    async fn get_package_vulnerabilities(&self, package_name: &str, version: &str) -> PackerResult<Vec<EnhancedVulnerability>> {
        let feeds = self.vulnerability_feeds.read().await;
        let mut all_vulnerabilities = Vec::new();

        for feed in feeds.values() {
            for vuln in &feed.vulnerabilities {
                if vuln.package_name == package_name && self.is_version_affected(version, &vuln.affected_versions) {
        ...
        all_vulnerabilities.sort_by(|a, b| {
            match (a.severity.clone(), b.severity.clone()) {
                (VulnerabilitySeverity::Critical, VulnerabilitySeverity::Critical) => b.published_date.cmp(&a.published_date),
                (VulnerabilitySeverity::Critical, _) => std::cmp::Ordering::Less,
                (_, VulnerabilitySeverity::Critical) => std::cmp::Ordering::Greater,
                (VulnerabilitySeverity::High, VulnerabilitySeverity::High) => b.published_date.cmp(&a.published_date),
    ...
    /// Check if a version is affected by vulnerability version ranges
    ...
    /// Check if version falls within a specific range
    fn version_in_range(&self, version: &str, range: &VersionRange) -> bool {
        ...
        match (&range.min_version, &range.max_version) {
            (Some(min), Some(max)) => {
                let min_check = if range.inclusive_min { version >= min.as_str() } else { version > min.as_str() };
                let max_check = if range.inclusive_max { version <= max.as_str() } else { version < max.as_str() };
                min_check && max_check
            }
            (Some(min), None) => {
                if range.inclusive_min { version >= min.as_str() } else { version > min.as_str() }
            }
            (None, Some(max)) => {
                if range.inclusive_max { version <= max.as_str() } else { version < max.as_str() }
            }
            (None, None) => true,
        }
    }

    /// Check for active threats targeting the package
    async fn check_active_threats(&self, package_name: &str) -> PackerResult<Vec<ActiveThreat>> {
        ...
        let active_threats = threat_intel.active_campaigns.iter()
            ...
            .map(|campaign| ActiveThreat {
                ...
                indicators: Vec::new(), // Would be populated from campaign data
            })
            .collect();
    ...
    /// Get exploit information for vulnerabilities
    async fn get_exploit_information(&self, vulnerabilities: &[EnhancedVulnerability]) -> PackerResult<Vec<ExploitInfo>> {
    ...
    async fn generate_security_recommendations(
        ...
        let critical_with_exploits: Vec<_> = vulnerabilities.iter()
            .filter(|v| matches!(v.severity, VulnerabilitySeverity::Critical) && v.exploit_available)
            .collect();
        ...
        if !active_threats.is_empty() {
            recommendations.push(SecurityRecommendation {
                ...
                description: format!(
                    ...
                    package.name, active_threats.len()
                ),
                actions: vec![
    ...
    async fn update_nvd_feed(&self, feed: &mut VulnerabilityFeed) -> PackerResult<()> {
        ...
        if response.status().is_success() {
            ...
            info!("Updated NVD feed with {} vulnerabilities", feed.vulnerabilities.len());
        }
    ...
    async fn update_osv_feed(&self, feed: &mut VulnerabilityFeed) -> PackerResult<()> {
        // OSV feed update implementation
        let client = reqwest::Client::new();
        ...
        info!("Updated OSV feed with {} vulnerabilities", feed.vulnerabilities.len());
        Ok(())
    }

    async fn update_github_feed(&self, feed: &mut VulnerabilityFeed) -> PackerResult<()> {
        // GitHub Security Advisories feed update
        let client = reqwest::Client::new();
        ...
        if response.status().is_success() {
            ...
            info!("Updated GitHub feed with {} vulnerabilities", feed.vulnerabilities.len());
        }
    ...
    async fn update_cisa_feed(&self, feed: &mut VulnerabilityFeed) -> PackerResult<()> {
        ...
        if response.status().is_success() {
            ...
            info!("Updated CISA feed with {} vulnerabilities", feed.vulnerabilities.len());
        }
    ...
    async fn update_arch_security_feed(&self, _feed: &mut VulnerabilityFeed) -> PackerResult<()> {
        // Arch Linux Security feed update
        info!("Updating Arch Security feed");
        // Implementation would fetch from Arch security announcements
        Ok(())
    }

    async fn update_custom_feed(&self, feed: &mut VulnerabilityFeed) -> PackerResult<()> {
        // Custom feed update implementation
    ...
    async fn update_threat_intelligence(&self) -> PackerResult<()> {
        ...
        // This would integrate with commercial threat intel feeds
    ...
    async fn update_exploit_database(&self) -> PackerResult<()> {
        ...
        // Update from exploit databases like Exploit-DB, Metasploit, etc.
    ...
    fn parse_nvd_response(&self, data: &serde_json::Value) -> PackerResult<Vec<EnhancedVulnerability>> {
        let mut vulnerabilities = Vec::new();
    ...
    fn parse_nvd_vulnerability(&self, data: &serde_json::Value) -> PackerResult<Option<EnhancedVulnerability>> {
        ...
        if let Some(cve_id) = data["cve"]["id"].as_str() {
            ...
            return Ok(Some(EnhancedVulnerability {
                id: cve_id.to_string(),
                cve_id: Some(cve_id.to_string()),
                package_name: "unknown".to_string(), // Would need to extract from configurations
                affected_versions: Vec::new(),
                severity,
                cvss_score,
                cvss_vector: None,
    ...
    fn parse_osv_response(&self, data: &serde_json::Value) -> PackerResult<Option<Vec<EnhancedVulnerability>>> {
    ...
    fn parse_osv_vulnerability(&self, data: &serde_json::Value) -> PackerResult<Option<EnhancedVulnerability>> {
        // Parse individual OSV vulnerability
        if let Some(id) = data["id"].as_str() {
            let summary = data["summary"].as_str().unwrap_or("No summary available").to_string();
    ...
    fn parse_github_response(&self, data: &serde_json::Value) -> PackerResult<Vec<EnhancedVulnerability>> {
    ...
    fn parse_github_advisory(&self, data: &serde_json::Value) -> PackerResult<Option<EnhancedVulnerability>> {
    ...
    fn parse_cisa_response(&self, data: &serde_json::Value) -> PackerResult<Vec<EnhancedVulnerability>> {
    ...
    fn parse_cisa_vulnerability(&self, data: &serde_json::Value) -> PackerResult<Option<EnhancedVulnerability>> {
        // Parse CISA vulnerability format
        if let Some(cve_id) = data["cveID"].as_str() {
            let description = data["vulnerabilityName"].as_str().unwrap_or("No description available").to_string();
            ...
            return Ok(Some(EnhancedVulnerability {
                ...
                package_name: "unknown".to_string(),
                affected_versions: Vec::new(),
                severity: VulnerabilitySeverity::High, // CISA KEV are high priority
                cvss_score: None,
                cvss_vector: None,
                epss_score: None,
                description,
                ...
                last_modified: Utc::now(),
                fixed_versions: Vec::new(),
                exploit_available: true, // CISA KEV indicates known exploitation
                exploit_maturity: ExploitMaturity::Functional,
                threat_actors: Vec::new(),
                attack_patterns: Vec::new(),
                mitigation_strategies: Vec::new(),
...
impl GPGManager {
    pub fn new(config: Config) -> Self {
        let keyring_path = config.gpg_config.keyring_path
            .as_ref()
        ...
        Self {
            ...
            keyservers: config.gpg_config.trusted_keyservers.clone().into_iter().collect(),
            config,
        }
    }

    /// Enhanced GPG signature verification with key validation
    pub async fn verify_package_signature(&self, package: &Package, file_path: &std::path::Path) -> PackerResult<SignatureVerificationResult> {
    ...
    async fn verify_signature_with_gpg(&self, _file_path: &std::path::Path, _signature: &str) -> PackerResult<GPGVerificationResult> {
        // Implementation would call GPG binary or use GPG library
...
impl RiskCalculator {
    ...
    pub async fn calculate_comprehensive_risk(
        ...
        let exploit_score = if vulnerabilities.iter().any(|v| v.exploit_available) {
            let max_maturity = vulnerabilities.iter()
        ...
        let threat_score = if threat_intel.active_campaigns.iter()
        ...
        let business_impact_score = vulnerabilities.iter()
        ...
        let newest_vuln_age = vulnerabilities.iter()
        ...
        let overall_score = cvss_component + epss_component + exploit_component + 
                       threat_intel_component + business_impact_component + temporal_component;
