...
impl NativePackageDatabase {
    ...
    pub fn search(&self, query: &str) -> Vec<&CorePackage> {
        ...
        results.sort_by(|a, b| {
            ...
            match (a_exact, b_exact) {
                ...
                _ => {
                    match (a.source_type == SourceType::Official, b.source_type == SourceType::Official) {
                        (true, false) => std::cmp::Ordering::Less,
                        (false, true) => std::cmp::Ordering::Greater,
                        _ => a.name.cmp(&b.name),
                    }
    ...
    pub async fn add_package(&mut self, package: CorePackage) -> PackerResult<()> {
        match package.source_type {
            SourceType::Official => {
                self.official_db.packages.insert(package.name.clone(), package);
    ...
    async fn fetch_popular_aur_packages(&self, client: &reqwest::Client) -> PackerResult<Vec<CorePackage>> {
                        ...
                        match response.json::<serde_json::Value>().await {
                            Ok(json) => {
                                if let Some(results) = json["results"].as_array() {
                                    for result in results.iter().take(20) { // top 20 from each search term should be enough
        ...
        for package in packages {
            unique_packages.entry(package.name.clone()).or_insert(package);
        }
    ...
    async fn sync_aur_by_categories(&mut self, client: &reqwest::Client) -> PackerResult<usize> {
        ...
        let categories = vec![
            "browser", "editor", "terminal", "media", "development", "security",
            "network", "system", "graphics", "office", "science", "game"
        ];
        
        for category in categories {
            let url = format!("https://aur.archlinux.org/rpc/?v=5&type=search&by=popularity&arg={}", category);
    ...
    async fn sync_repo_database(&mut self, client: &reqwest::Client, repo: &str) -> PackerResult<usize> {
        ...
        if !response.status().is_success() {
            return Err(crate::error::PackerError::NetworkError(
                format!("failed to fetch {}.db: {}", repo, response.status())
            ));
        }
    ...
    async fn extract_and_parse_db(&mut self, db_file: &std::path::Path, repo: &str) -> PackerResult<usize> {
        use std::process::Command;
        
        let extract_dir = db_file.parent().unwrap().join(format!("{}_extracted", repo));
        ...
        if !output.status.success() {
            warn!("failed to extract database: {}", String::from_utf8_lossy(&output.stderr));
            return Ok(0);
        }
    ...
    async fn parse_package_dir(&self, package_dir: &std::path::Path) -> PackerResult<Option<CorePackage>> {
    ...
    fn add_essential_packages(&mut self) {
        ...
        let essential_packages = vec![
            ...
            ("tree", "Directory listing program displaying tree-like structure"),
    ...
    async fn load_from_disk(&mut self) -> PackerResult<()> {
        ...
        info!("Loaded database: {} official packages, {} AUR packages", 
